...
function getLabel(context: vscode. SignatureHelpContext): string {
    ...
    switch (context.triggerKind) {
        case vscode. Invoke:            parts.push('invoke'); break;
        case vscode. ContentChange:     parts.push('change'); break;
        case vscode. TriggerCharacter:  parts.push('character'); break;
    }
...
When VS Code started, JavaScript wasn't what it is today. For instance, there was no [native promise](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise) and back then the team decided to use `WinJS. Fast forwarding to today, the situation is different: native promises are real and <code>WinJS. Promise` is obsolete.

Because of that, we are in the progress of migrating away from `WinJS. This is no easy undertaking because of differences in the API and the runtime behavior. We have started at around 10000 references to <code>WinJS. Promise` and are down to 1471, of which ~500 are instantiations and not plain type annotation. We are confident we will finish this epic journey soon!
